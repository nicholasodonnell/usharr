...
export class MovieService {
  ...
  async deleteWhereNotIds(ids: number[]): Promise<void> {
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        // un-delete movies in the provided list of ids
        // chunked to avoid "too many SQL variables" error
        for (const idsChunk of chunk<number>(ids, 500)) {
          await tx.movie.updateMany({
  ...
  async getForRule(
    ...
    try {
      ...
      const where: Prisma.MovieWhereInput = {
        appearsInList: !ignoreDynamic
          ? (appearsInList ?? undefined)
          : undefined,
        deleted: false,
        downloadedAt:
  ...
  async getNotDeleted(
    ...
    try {
      return await this.findMany({ orderBy, where: { deleted: false } })
    } catch (e) {
  ...
  async getUnmonitored(): Promise<Movie[]> {
    try {
      ...
      const notDeletedMovies: Movie[] = await this.getNotDeleted({
        orderBy: {
          downloadedAt: 'desc',
        },
      })

      const monitoredMoviesIds: Set<number> = new Set(
        monitoredMovies.map((movie: Movie) => movie.id),
      )

      return notDeletedMovies.filter(
        (movie: Movie) =>
          !monitoredMoviesIds.has(movie.id) && movie.ignored === false,
      )
    } catch (e) {
